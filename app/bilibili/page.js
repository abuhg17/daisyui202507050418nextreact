...
const BilibiliPage = () => {
  ...
  const [bilibilis, setBilibilis] = useState([]);
  ...
  const loadData = async () => {
    ...
    for (let i = 0; i < arrs.length; i++) {
      ...
      try {
        ...
        newBilibilis.push(json);
      } catch (e) {
        newBilibilis.push({ bvid, error: "載入失敗" });
      }
      setBilibilis([...newBilibilis]);
      setLoadedCount(i + 1);
      await new Promise((r) => setTimeout(r, 300));
    }

    setCache(newBilibilis);
    setIsLoading(false);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
      <h1 className="text-3xl font-bold mb-6 text-center">Bilibili Page</h1>
